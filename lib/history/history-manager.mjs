...
export function HistoryManager(postId, apiClient) {
  let historyCandidateLastSavedState = new DocumentModelNode();
  let historyCandidateState = new DocumentModelNode();
  ...
  function flattenHistory(historyState) {
    ...
    historyState.forEach(({ head, nodes }) => {
      lastHead = head;
      nodes.forEach((nodeOrId) => {
        if (!nodeOrId) {
          console.error(historyState);
    ...
    return { head: lastHead, nodes: nodesById };
  }

  function flushPendingHistoryLogEntry() {
    if (!historyCandidateState.id) {
      return;
    }
    const historyEntry = fromJS({
      ...
      historyState: {
        head: undefined,
        nodes: { [historyCandidateState.id]: historyCandidateState },
      },
    });
    console.info(
  ...
  function appendToHistoryLogWhenNCharsAreDifferent({
    ...
    if (historyCandidateState.id !== nodeAfterUpdate.id) {
      if (historyCandidateState.id) {
    ...
    if (
      moreThanNCharsAreDifferent(
        historyCandidateState[comparisonKey],
        historyCandidateLastSavedState[comparisonKey]
      )
    ) {
      flushPendingHistoryLogEntry();
    }
