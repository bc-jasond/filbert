  ...
  function prepareHistoryEntry({
    ...
    function flattenHistory(historyState, shouldExecute = true) {
      ...
      history.forEach((state) => {
        ...
        if (!state[primaryKey]) {
          if (!state[otherKey]) {
            //console.warn("bad history", state)
            return
          }
...
export function getLastInsertedNodeIdFromHistory(history) {
  return [...history]
      // falsy unexecuteState means a newly inserted node, get the last one of those
      .filter(({unexecuteState}) => !unexecuteState)
      .pop().executeState.get('id');
}
