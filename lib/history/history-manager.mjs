...
import { documentModelFromJS, meta } from '@filbert/document';
...
export function HistoryManager(postId, apiClient) {
  ...
  async function undoRedoInternal(isUndo = true) {
    ...
    data.documentModel = documentModelFromJS(data.documentModel);
    data.updatedPost = fromJS(data.updatedPost);

    if (isEmpty(data.documentModel)) {
      return {};
    }
