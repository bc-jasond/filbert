...
export function fixOrphanedNodes(nodesById) {
  ...
  function getEndNode(nodeId) {
    ...
    let last;
    const seen = new Set()
    while (current && current.get('next_sibling_id') && !seen.has(current.get('next_sibling_id'))) {
  ...
  const orphanIdsDescendingByListLength = orphanedNodes
      ...
      .sort(({len: lenLeft}, {len: lenRight}) => lenLeft > lenRight ? -1 : 1)
      .map(({id}) => id);
  ...
  for (let i = 1; i < orphanIdsDescendingByListLength.length; i++) {
    ...
    nodesById = nodesById.set(prevEndNode.get('id'), prevEndNode.set('next_sibling_id', currentId))
  }
...
export function getOrphanedNodes(nodesById) {
  ...
  nodesById.forEach((node) => {
    ...
    if (node.get('next_sibling_id') && nodesById.get(node.get('next_sibling_id'))) {
...
export function getFirstNode(nodesById) {
  ...
  if (difference.length !== 1) {
    console.error(
      `${nodesById.get(difference[0]).get('post_id')} DocumentError.getFirstNode() - more than one node isn't pointed to by another node!`,
      difference.reduce((acc, id) => `${acc}${id} - ${getNodeListLength(nodesById, id)} nodes, `, '')
    );
  }
  ...
  function update(node) {
    ...
    if (newNextId && !newNextNode) {
      if (prevNextId && prevNextNode) {
        console.warn("DocumentModel.update() bad next_sibling_id - found previous node, using previous value", node.toJS())
        node = node.set('next_sibling_id', prevNextId)
      } else {
        console.warn("DocumentModel.update() bad next_sibling_id - unsetting", node.toJS())
