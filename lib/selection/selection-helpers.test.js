...
const { fromJS, Map, Record } = immutable;
...
function getTestSelectionAtIdx(nodeModel, idx) {
  const { head, selections } = getSelections(nodeModel);
  let current = head;
  let idxCount = 0;
  while (idxCount < idx) {
    idxCount += 1;
...
describe('adjustSelectionOffsetsAndCleanup', () => {
  test('delete all highlighted characters up to caret (when "end" in handleBackspace)', () => {
    ...
    expect(toArray(modelAdjusted.getIn(['meta', 'selections']))).toEqual(
      toArray(expectedSelections)
    );
  });
  test('delete all highlighted characters up to caret (up to edge of a selection)', () => {
    ...
    const testModel = nodeModelWithSelections
      .set('content', 'and some paragraph for  measure?')
      .setIn(
        ['meta', 'selections'],
        makeSelections([
          [9],
  ...
  test('delete all characters (and all Selections)', () => {
    ...
    expect(modelAdjusted.getIn(['meta', 'selections'])).toBeFalsy();
  });
...
describe('getSelectionByContentOffset', () => {
  test('finds existing selection, preserves existing formats', () => {
    ...
    expect(updatedNodeModel.getIn(['meta', 'selections'])).toEqual(
      expectedSelections
    );
  ...
  test('creates new selection - replaces first (head) selection - empty selections', () => {
    const expectedSelectionId = s4();
    const expectedSelections = makeSelections([
      [8, { key: SELECTION_ID, value: expectedSelectionId }],
      [],
    ]);
    const testModel = nodeModelWithSelections.setIn(
      ['meta', 'selections'],
  ...
  test('creates new selection - completely within last selection', () => {
    ...
    const expectedSelections = makeSelections([
      ...
      [9, SELECTION_ACTION_CODE],
      [3],
      [3, { key: SELECTION_ID, value: expectedSelectionId }],
      [],
    ]);
    const testModel = nodeModelWithSelections.setIn(
      ['meta', 'selections'],
...
describe('replaceSelection', () => {
  test('replaces (updates) a "matched" Selection in the middle', () => {
    ...
    const updatedSelection = getTestSelectionAtIdx(
      nodeModelWithSelections,
      2
    ...
    expect(toArray(updatedModel.getIn(['meta', 'selections']))).toEqual(
      toArray(expectedSelections)
    );
  });
  test('replaces and merges Selections - in the middle', () => {
    const updatedSelection = getTestSelectionAtIdx(
      nodeModelWithSelections,
      2
    ).set(SELECTION_ACTION_ITALIC, false);
    const expectedSelections = makeSelections([
      [3, SELECTION_ACTION_SITEINFO],
      [10],
      [9, SELECTION_ACTION_CODE],
  ...
  test('replaces a Selection (no merge) - first (head) selection', () => {
    const updatedSelection = getTestSelectionAtIdx(nodeModelWithSelections, 0)
  ...
  test('should throw with bad Selection', () => {
    expect(() => {
      replaceSelection(nodeModelWithSelections, new Selection());
    }).toThrow();
  });
});
