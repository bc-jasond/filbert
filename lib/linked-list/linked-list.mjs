...
export function validate({
  ...
  nodes.forEach((node, nodeId) => {
    ...
    seen.add(nodeId);
    if (node.get(LINKED_LIST_NODE_NEXT_ID)) {
      if (seenNext.has(node.get(LINKED_LIST_NODE_NEXT_ID))) {
        throw new Error('LinkedList - cycle detected!');
      }
      if (!nodes.has(node.get(LINKED_LIST_NODE_NEXT_ID))) {
        throw new Error('LinkedList - bad next id, not found');
      }
      seenNext.add(node.get(LINKED_LIST_NODE_NEXT_ID));
    }
  });
...
export function getNext(linkedList, nodeOrId) {
  ...
  return getNode(linkedList, node.get(LINKED_LIST_NODE_NEXT_ID));
}
...
export function getPrev(linkedList, nodeOrId) {
  ...
  while (current.size > 0) {
    if (
      current.get(LINKED_LIST_NODE_NEXT_ID) === node.get(LINKED_LIST_NODE_ID)
    ) {
      return current;
    }
    current = getNext(linkedList, current);
...
export function replace(linkedList, node) {
  ...
  if (
    node.get(LINKED_LIST_NODE_NEXT_ID) &&
    !getNode(linkedList, node.get(LINKED_LIST_NODE_NEXT_ID)).size === 0
  ) {
...
export function insertAt(linkedList, data, index) {
  ...
  if (index === 0) {
    ...
    linkedList = linkedList
      .set(LINKED_LIST_HEAD_ID, node.get(LINKED_LIST_NODE_ID))
  ...
  // adding an element
  if (current) {
    node = node.set(LINKED_LIST_NODE_NEXT_ID, current.get(LINKED_LIST_NODE_ID));
  }
  linkedList = linkedList.setIn(
    [LINKED_LIST_NODES_MAP, node.get(LINKED_LIST_NODE_ID)],
    node
  );
  prev = prev.set(LINKED_LIST_NODE_NEXT_ID, node.get(LINKED_LIST_NODE_ID));
...
export function removeAt(linkedList, index) {
  ...
  if (index === 0) {
    linkedList = linkedList.set(
      LINKED_LIST_HEAD_ID,
      getNext(linkedList, current).get(LINKED_LIST_NODE_ID)
    );
  } else {
    ...
    prev = prev.set(
      LINKED_LIST_NODE_NEXT_ID,
      current.get(LINKED_LIST_NODE_NEXT_ID)
    );
    linkedList = replace(linkedList, prev);
  }
  linkedList = linkedList.removeIn([
    LINKED_LIST_NODES_MAP,
    current.get(LINKED_LIST_NODE_ID),
  ]);
  return linkedList;
}
