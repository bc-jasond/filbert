...
export function getNextId(node) {
  ...
  return node.get(LINKED_LIST_NODE_NEXT_ID);
}
...
export function copy(node) {
  return fromJS(node.toJS());
}

export function validate(linkedListArg) {
  let headId;
  let nodesArg;
  let nodes;
  if (Map.isMap(linkedListArg)){
    headId = linkedListArg.get(LINKED_LIST_HEAD_ID);
    nodes = linkedListArg.get(LINKED_LIST_NODES_MAP);
  } else {
    ({
      [LINKED_LIST_HEAD_ID]: headId,
      [LINKED_LIST_NODES_MAP]: nodesArg = {},
    } = linkedListArg);
    nodes = fromJS(nodesArg);
  }
  if (!Map.isMap(nodes)) {
...
export function nodes(linkedList) {
  return linkedList.get(LINKED_LIST_NODES_MAP);
}
