...
async function main() {
  try {
    console.info(
      figlet.textSync(
        'document model format converter',
        {
          //font: 'Doh',
        })
    )
    ...
    const [newPostId] = await knex
     .insert({ user_id: 1, title: 'placeholder post', canonical: 'some-url-here' })
     .into('post');
    
    let updates = Map();
    // updates.push(["34ab", { post_id: newPostId, node: Map() }])
    const nodesByParentId = fromJS(JSON.parse(JSON.stringify(contentNodes)));
    const rootId = nodesByParentId.get("null").first().get('id')
    let last;
    ...
    function inner(nodeId) {
      ...
      nodes.forEach(node => {
        node = node.delete('parent_id').delete('position').set('post_id', newPostId);
        if (nodesByParentId.get(node.get('id'))) {
          inner(node.get('id'));
          return;
        }
        if (node.get('type') === 'codesection') {
          const lines = node.getIn(['meta', 'lines']);
          lines.forEach(line => {
            const newNode = getMapWithId({ content: line, type: 'pre', post_id: newPostId })
            if (last) {
              updates = updates.set(last.get('id'), last.set('next_sibling_id', newNode.get('id')))
            }
        ...
        if (last) {
          updates = updates.set(last.get('id'), last.set('next_sibling_id', node.get('id')))
        }
