...
async function uploadFileToBucket(bucket, fileNameWithAbsolutePath) {
  ...
  return upload({Bucket: bucket, Body: fileStream, Key: path.basename(fileNameWithAbsolutePath)})
}

async function listKeysForBucket(bucket, { sortOrder } = { sortOrder: 'desc' }) {
  const response = await listObjects({Bucket: bucket});
  const comparisonFn = sortOrder === 'desc'
    ? (a, b) => a > b ? -1 : 1
    : (a, b) => a > b ? 1 : -1;
  
  if (response.IsTruncated) {
    console.warn('listKeysForBucket', 'local options: ', options, 'response ', response);
  }
